...
# o manager = Manager()
# o dicionario_fibonacci_par = manager.dict()
# o gerar_chaves(dicionario_fibonacci_par, qtd_valores)
# o [p.join() for p in lista_de_processos]

# Task: Comment out the instructions show_dictionary(dictionary_fibonacci), change
# to 10000 the value of qty_values and run the program.

# Task: Perform a critical analysis of the work performed by each of the processes created
# relating to the number of terms to be calculated and the use of the "queue".

# Answer: We can verify that as we increase the qty of values the processes start to have a more "uniform" workload,
# the frequency with which the processes access the queue verifies this
